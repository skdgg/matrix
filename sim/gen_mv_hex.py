...
def make_tests(n=100, seed=0x1234, val_range=(-2.0, 2.0)):
    ...
    for _ in range(n):
        ...
        out_u32 = mv4x4_fp32_trunc_hw(M, v)
        tests.append((M, v, out_u32))
...
def main():
    ...
    for (M, v, out_u32) in tests:
        # input layout (20 words): m00..m33 (row-major), vx,vy,vz,vw
        for r in range(4):
            for c in range(4):
        ...
        # output layout (4 words): ox,oy,oz,ow
        for r in range(4):
    ...
    print("Generated (HW-trunc golden, NO id):")
    print(f"  {OUTDIR/'mv_in.hex'}  ({len(in_words)} words = {N} cases * 20)")
    print(f"  {OUTDIR/'mv_out.hex'} ({len(out_words)} words = {N} cases * 4)")
    print("Format:")
    print("  mv_in.hex : m00..m33, vx,vy,vz,vw (20 lines per case)")
    print("  mv_out.hex: ox,oy,oz,ow           (4 lines per case)")
